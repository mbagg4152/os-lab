...
//  Must be called with interrupts disabled to avoid the caller being rescheduled between reading lapicid and running
//  through the loop.
...
// Look in the process table for an UNUSED proc. If found, change state to EMBRYO and initialize state required to
...
void userinit(void) {
	...
	// this assignment to p->state lets other cores run this process. the acquire forces the above writes to be visible, and the lock is also needed because the assignment might not be atomic.
	acquire(&ptable.lock);
...
// Per-CPU process scheduler. Each CPU calls scheduler() after setting itself up. Scheduler never returns.  It
// loops, doing: - choose a process to run - swtch to start running that process - eventually that process transfers
...
// Print a process listing to console.  For debugging. Runs when user types ^P on console. No lock to avoid wedging a
// stuck machine further.
void procdump(void) {
	static char *states[] = {[UNUSED] = "unused", [EMBRYO] = "embryo", [SLEEPING]  = "sleep ", [RUNNABLE]  =
	"runnable", [RUNNING] = "run", [ZOMBIE] = "zombie"};
	int i;
	struct proc *p;
	char *state;
	uint pc[10];
...
// add this function at the end of proc.c look into scheduler method and modify the function according to the hints
// provided
